...
class View {
	...
	func setMinimumSize (_ size: Size) -> Self {
		return self
	}
	func setMaximumSize (_ size: Size) -> Self {
		return self
	}
	func setStyleSheet (_ style: String) -> Self {
		return self
	}
}

class VBoxLayout : View {
	init (_ widgets:[some View]) {

	}
	deinit {
	}
	func setSpacing (_ value: Int) -> Self {
		return self
	}
}

class HBoxLayout : View {
	init (_ widgets:[some View]) {
...
class List : View {
	...
	func setColumnCount (_ count: Int) -> Self {
		 return self 
	}
	func setRowCount (_ count: Int) -> Self {
		 return self 
	}
	func setHorizontalHeaderLabels (_ labels: [String]) -> Self {
		 return self 
	}
	func setVerticalHeaderLabels (_ labels: [String]) -> Self {
		 return self 
	}
	func setCellClickedHandler (_ block: (_ : List, _ row:Int, _ column:Int)-> Void) -> Self { 
		return self
	}
}
...
class Label : View {
	...
	func setWordWrap (_ wrap: Bool) -> Self {
		return self;
	}
	func setAlignment (_ alignment: Int) -> Self {
		return self;
	}
	func setText (_ text: String) -> Self {
		return self
	}
	func setImage (_ image: QImage) -> Self {
		return self
	}
}
...
class Button : View {
	...
	func setClickedHandler (_ block: (_ : Button)-> Void) -> Self { 
		return self
	}
	func setFlat (_ value: Bool) -> Self {
		return self;
	}
	func setDefault (_ value: Bool) -> Self {
		return self;
	}
	func setText (_ text: String) -> Self {
		return self
...
class RadioButton : View {
	...
	func setToggledHandler (_ block: (_ : RadioButton)->Void) -> Self { 
		return self
	}
}
...
class CheckBox : View {
	...
	func setStateChangedHandler (_ block: (_ : CheckBox)->Void) -> Self { 
		return self
	}
}

class WebEngineView : View {
	override init () {
	}
	deinit {
	}
...
class SampleBoxLayoutApp : ApplicationDelegate {
	...
	let ui = [
		HBoxLayout([
			...
			VBoxLayout([
				View()
				.setMinimumSize (Size(200,1)),

				Label ("A Label w/word wrap, left/bottom aligned")
				.setWordWrap (true)
				.setAlignment (Qt.AlignLeft + Qt.AlignBottom)
				.setStyleSheet ("background-color : cyan; color : #008;"),

				Button ("Regular Button")
				.setClickedHandler ({ view in
					let string = view.button1!.text()
					print ("Button \"\(string)\" clicked.")
					window.setWindowTitle ("This is a test")
					SwiftQt.infoPopup ("Hello World")
				})
				.setStyleSheet ("background-color : white; color : #080;"),

				Button ("Flat Button")
				.setFlat (true)
				.setClickedHandler ({ view in
					let string = view.button2!.text()
					print ("Button \"\(string)\" clicked.")
				...
				.setToggledHandler ({ view in
					let value = view.isChecked()
					print ("The radio button was toggled, isChecked=\(value)")
				}),

				CheckBox ("CheckBox")
				.setStateChangedHandler ({ view in
					let checkState = view.checkState() // 3 possible values
					var string = "?"
					switch (checkState) {
					case t.Checked: 
						string = "checked"
					case t.PartiallyChecked: 
			...
			.setVerticalHeaderLabels (["fee", "fie", "foe", "fum"])
			.setCellClickedHandler ({ view, row, column in
				let widget = view.cellWidget(row, column)
				if widget != nil {
					print ("Got widget!")
				} else {
